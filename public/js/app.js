...
window.showNotification = function (type, title, message) {
  ...
  const defaultMessage =
    type === "success" ? "Operasi berhasil." : "Silakan coba lagi.";
      ...
      <div class="flex w-full items-center justify-between">
        ...
        <div>
          <button class="hover:text-danger text-[#ACACB0] notification-close-btn">
             <svg width="24" height="24" viewBox="0 0 24 24" class="fill-current"><path fill-rule="evenodd" clip-rule="evenodd" d="M18.8839 5.11612C19.372 5.60427 19.372 6.39573 18.8839 6.88388L6.88388 18.8839C6.39573 19.372 5.60427 19.372 5.11612 18.8839C4.62796 18.3957 4.62796 17.6043 5.11612 17.1161L17.1161 5.11612C17.6043 4.62796 18.3957 4.62796 18.8839 5.11612Z" /><path fill-rule="evenodd" clip-rule="evenodd" d="M5.11612 5.11612C5.60427 4.62796 6.39573 4.62796 6.88388 5.11612L18.8839 17.1161C19.372 17.6043 19.372 18.3957 18.8839 18.8839C18.3957 19.372 17.6043 19.372 17.1161 18.8839L5.11612 6.88388C4.62796 6.39573 4.62796 5.60427 5.11612 5.11612Z" /></svg>
          </button>
        </div>
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  router.addRoute("/dashboard", async () => {
    ...
    // --- FIX: Re-execute scripts di dashboard.html ---
    ...
    // Panggil fungsi loadDashboard secara manual jika sudah ada di window
    setTimeout(() => {
  ...
  router.addRoute("/questionnaire", async () => {
    ...
    // --- FIX: Re-execute scripts di questionnaire.html ---
    // Agar event listener submit form berfungsi
    const scripts = app.querySelectorAll("script");
    scripts.forEach((oldScript) => {
      const newScript = document.createElement("script");
      Array.from(oldScript.attributes).forEach((attr) =>
  ...
  // 7. PRODUCTS CATALOG (DITAMBAHKAN)
  router.addRoute("/products", async () => {
    const app = document.getElementById("app");
    const response = await fetch("/pages/products.html");
    app.innerHTML = await response.text();

    // Re-execute scripts untuk products.html agar filter berfungsi
    const scripts = app.querySelectorAll("script");
    scripts.forEach((oldScript) => {
      const newScript = document.createElement("script");
      Array.from(oldScript.attributes).forEach((attr) =>
    ...
    // === FIX: Panggil fungsi init secara manual dengan delay sedikit ===
    setTimeout(() => {
